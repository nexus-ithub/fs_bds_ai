...
import { type ConsultRequest, HDivider, Pagination, getJibunAddress, Button, DotProgress, Spinner } from "@repo/common";
...
export default function ConsultRequest() {
  ...
  const handleUpdate = async (action: string) => {
    ...
    try{
      const response = await axiosInstance.put("/design-consult", { action, ids: selectedIds });
          ...
          <>
            ...
            <table className="w-full">
              <thead className="text-text-03 bg-surface-second text-left">
                <tr>
                  <th className="pl-[12px] py-[14px] w-[48px]">
                    <input
                      type="checkbox"
              ...
              <tbody>
                {list?.map((item : ConsultRequest, index : number) => (
                  ...
                  >
                    ...
                    <td className="pl-[16px] cursor-pointer" onClick={() => {setSelectedItem(item)}}>{item.user.email}</td>
                    <td className="pl-[12px] cursor-pointer" onClick={() => {setSelectedItem(item)}}>{item.user.name}</td>
                    <td className="pl-[12px] cursor-pointer" onClick={() => {setSelectedItem(item)}}>{item.user.phone}</td>
                    <td className="pl-[12px] cursor-pointer" onClick={() => {setSelectedItem(item)}}>{getJibunAddress(item.land)}</td>
                    <td className="pl-[12px] truncate max-w-[80px] cursor-pointer" onClick={() => {setSelectedItem(item)}}>{item.content}</td>
                    <td className="pl-[12px] cursor-pointer w-[144px]" onClick={() => {setSelectedItem(item)}}>{format(item.createdAt, 'yyyy.MM.dd HH:mm:ss')}</td>
                    <td 
