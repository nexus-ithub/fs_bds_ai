...
export async function PUT(req: Request) {
  ...
  if (action === "pending") {
    const result = await ConsultRequestModel.updatePending(ids);
    if (result) {
    ...
    const result = await ConsultRequestModel.updateComplete(ids);
    if (result) {
    ...
    const result = await ConsultRequestModel.updateDelete(ids);
    if (result) {
