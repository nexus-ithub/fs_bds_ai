...
export class AuthModel {
  ...
  static async updateRefreshToken(adminId: string, refreshToken: string | null, expiresAt: Date): Promise<void> {
    ...
    const rows: any[] = await db.query(
      "SELECT * FROM admin_refresh_tokens WHERE admin_id = ? ORDER BY expires_at DESC LIMIT 1",
      [adminId]
    );
  ...
  static async findRefreshToken(adminId: string): Promise<any> {
    const [rows] = await db.query<RowDataPacket[]>(
      "SELECT token, expires_at as expiresAt FROM admin_refresh_tokens WHERE admin_id = ? ORDER BY expires_at DESC LIMIT 1",
      [adminId]
    );
