...
export class SearchModel {
  ...
  static async bmReportSearch(userId: number, query: string, page: number, size: number): Promise<SearchResult[] | null> {
    try {
      const normalizedQuery = normalize(query);
      
      console.log('search query: ', query)
      const result = await db.query<SearchResult>(
        `SELECT id, jibun, road, building_name as buildingName FROM address_search WHERE key_jibun LIKE ? OR key_road LIKE ? OR key_building LIKE ? AND user_id = ? LIMIT ? OFFSET ?`,
